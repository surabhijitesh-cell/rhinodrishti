...
@api_router.post("/knowledge-graph/build")
async def build_kg(background_tasks: BackgroundTasks):
    """Trigger knowledge graph build from all processed intelligence items."""
...
@api_router.get("/knowledge-graph/stats")
async def kg_stats():
...
@api_router.get("/knowledge-graph/actors/{actor_name}")
async def kg_actor_detail(actor_name: str):
    """Get detailed profile for a specific actor."""
    import re as _re
...
@api_router.get("/knowledge-graph/locations")
...
@api_router.get("/knowledge-graph/edges")
async def kg_edges(
    ...
    """Get actor-location edges. Filter by actor, location, or minimum co-occurrence count."""
...
@api_router.get("/knowledge-graph/network")
async def kg_network(limit: int = Query(50, ge=10, le=200)):
    """Get network graph data (nodes + links) for visualization."""
    # Get top actors
    top_actors = await db.kg_actors.find(
    ...
    # Get locations connected to these actors
    loc_query = {}
    if actor_names:
